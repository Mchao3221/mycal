...
export async function summarizeDay(db: Env['mycalDB'], env: Env, dateKey: string): Promise<AiSummary> {
  ...
  const system = [
    ...
    '任务二 comment:一篇 250~400 字的中文分段点评,要有信息量、落到具体数字与条目,禁止客套空话。共四段,段间空一行,每段以方括号小标题开头:【今日总评】对照建议摄入与净差给一句明确结论(盈余/亏空多少、折合体重趋势);【饮食点评】点名 1~2 个具体条目,从蛋白质/蔬菜/精制碳水/油脂/加餐时机等角度谈结构性问题或亮点;【作息与活动】结合当日流水与运动条目观察久坐、压力、睡眠、活动量线索,记录太少就直说缺哪些信息;【明日建议】只给一条今天用得上、具体到食物/分量/动作/时间的可执行改法。不用 Markdown 语法(方括号小标题除外),语气像靠谱的朋友,直接友好。',
