...
- 🍽 **吃动打卡 + AI 每日汇总**:饮食/运动条目热量可留空,AI(OpenAI 兼容)补齐,并结合档案 BMR/TDEE 与当日流水、体重写出分段点评(右栏独立面板);
...
3. 主体一天一张卡**左右分栏**:左边记一天——**当日流水**(做了哪些事)→ **今日体重**(晨起空腹,一天一条)→ **饮食/运动条目**(热量可留空);右边 540px 是 **AI 每日汇总**栏(窄屏自动折到下方);
4. 点「生成今日汇总」→ AI 逐条估算缺失热量(不覆盖手动值),结合 BMR/TDEE、当日流水与体重(含近 7 日均值)给出摄入/消耗/净差,并写出 250~400 字分段点评(今日总评 / 饮食点评 / 作息与活动 / 明日建议);结果按天缓存,可清除重生成,生成后打卡列表自动刷新出 AI 估的热量;
5. 左栏「体重走势」点开曲线:7 日均主线 + 目标线 + BMI(中国标准分段)+ 按近期节奏估算达标周数。**体重录入只在右栏一处**——补录历史日就点日历那天再录,弹窗只负责看图与删除。
...
│                                 # JournalPanel / DiaryPanel(左侧记录区)/
│                                 # AiSummaryPanel(右侧 AI 点评)/ WeightToday /
...
- ✅ v0.4.2:**一日一卡改左右分栏**——AI 汇总独立为右栏面板,记录列表吃满左侧宽度;汇总 prompt 升级为 250~400 字四段式点评(今日总评/饮食点评/作息与活动/明日建议),要求结合当日具体数字
