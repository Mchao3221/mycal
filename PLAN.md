...
| 日历 | 「今天」高亮、选中日高亮、圆点标记(琥珀=本地待办,蓝=仅 ICS 日程) |
| 待办面板 | 日期标题、条目列表、统计「共 x 项 · 完成 y 项」、空状态提示 |
| 待办操作 | 添加(回车/按钮)、勾选完成/取消、删除;ICS 条目带「日程」徽标 |
| ICS 导入 | 顶部导入条:粘贴 http(s) 的 .ics 链接 → 服务端拉取解析 → 按天入库 |
| ICS 去重 | 同一 UID 在同一天只入库一次,重复导入同一链接零副作用 |
| 持久化 | SQLite(`data/mycal.db`),经 REST API 读写,前端不存任何数据 |
| 响应式 | 宽屏双栏,窄屏单列堆叠 |
...
2. 点任意日期 → 右侧面板切换为该天;
3. 输入 + 回车 → 添加待办;点复选框完成;点 ✕ 删除;
4. 顶部粘贴 .ics 链接 → 「导入」→ 显示「导入 n 条 · 跳过重复 n 条…」结果,日历出现蓝色圆点;
5. 琥珀圆点 = 有本地待办;蓝圆点 = 只有 ICS 日程;选中日圆点变白。
