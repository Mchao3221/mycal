...
- **一日一张卡,左右分栏**:左边记(流水 / 吃动打卡 / 体重),右边看(AI 汇总)——记录不被点评挤窄,点评有足够版面铺开;
...
| 当日 | **AI 每日汇总**(右栏独立面板):估缺省热量(不覆盖手动)+ 结合流水/体重/BMR/TDEE 写 250~400 字分段点评;按天缓存可重生成 |
| 体重 | **录入仅右栏一处**(点哪天录哪天,天然支持补录);曲线弹窗负责看图与管理:7 日均主线 + 原始点 + 目标线;90 天/1 年/全部;BMI 中国标准分段;按近期节奏估算达标周数;近期记录可删除 |
...
| 布局 | 桌面端一屏工作台:左栏 240px 导航/概览,主体一日一卡**左右分栏**(记录区 + 360px AI 汇总栏),各自内部滚动 |
...
3. 一次 `/chat/completions` 调用:为缺热量条目逐条估算(**只回填空值,不覆盖手动值**)+ **250~400 字分段中文点评**,按 `【今日总评】【饮食点评】【作息与活动】【明日建议】` 四段展开(每段之间空行,不写 Markdown),必须引用当日流水/体重/收支的具体数字而非泛泛而谈;回复按 JSON 解析(容忍 ``` 围栏),点评落库前截断到 2000 字;
...
│   └── components/             # LockScreen / Calendar / DayCell / DayView(左右分栏)/
│                               # JournalPanel / DiaryPanel(记录区)/ AiSummaryPanel(右栏点评)/
│                               # WeightToday / WeightSparkline / WeightModal /
...
3. **主体一天一张卡,左右分栏**:左侧记录区自上而下 = 日期标题 → 当日流水(输入框回车即记,行内 ✎ 改 ✕ 删)→ 今日体重(同日覆盖,显示较 7 日均)→ 吃动打卡;右侧 360px 独立栏 = 「AI 每日汇总」(窄屏自动折到下方);
4. **左栏下方两块**:本月概况(记录天数 / 条数 / 🔥连续,跟随所看月份)、体重走势卡(sparkline + 最新值,点击开曲线弹窗:90 天/1 年/全部切换、近期记录删除、BMI 中国标准、达标周数估算;**弹窗不含录入**,录入统一在右栏);
5. **打卡日记**:切饮食/运动,记「一碗牛肉面」即可;热量可留空由 AI 估;点条目上的 kcal 可手动修正(AI 不再覆盖)或清空回「待估」;
6. **AI 汇总**:先「档案」+「AI」配好 → 点「生成今日汇总」→ AI 估热量,并结合当日流水与体重写出分段点评(总评/饮食/作息与活动/明日建议);当天没有任何吃动但有流水也能生成;结果按天缓存,可清除重生成;生成后自动刷新打卡列表,AI 估的热量随即出现;
...
7. ✅ v0.4.2:**一日一卡改为左右分栏**:AI 汇总独立成右栏面板(`AiSummaryPanel`,日期切换即拉缓存、生成后回调刷新打卡),记录列表吃满左侧宽度不再被挤窄;汇总 prompt 升级为 250~400 字四段式点评,要求引用当日具体数字
8. ⬜ 下一轮候选:自然语言快记拆条、AI 周报/月报、导出 JSON 备份、AI 流式输出
